        ...
        dbPromise = openDB(DB_NAME, DB_VERSION, {
            upgrade(db) {
                if (!db.objectStoreNames.contains(STORE_NAME_SETS)) {
                    db.createObjectStore(STORE_NAME_SETS, { keyPath: "id", autoIncrement: true });
                }
                if (!db.objectStoreNames.contains(STORE_NAME_WORDS)) {
                    const wordsStore = db.createObjectStore(STORE_NAME_WORDS, { keyPath: "id", autoIncrement: true });
